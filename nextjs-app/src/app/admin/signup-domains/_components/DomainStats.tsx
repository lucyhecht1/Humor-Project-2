...
export async function DomainStats() {
  const { total, emailCount } = await fetchStats();
  return (
    <div className="mb-8 grid grid-cols-2 gap-4 sm:grid-cols-2">
      <StatCard label="Allowed Domains" value={total} />
      <StatCard label="Whitelisted Emails" value={emailCount} sub="individual overrides" />
    </div>
  );
}
