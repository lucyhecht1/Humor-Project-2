...
export function InlineDeleteButton({ id, label }: { id: number; label: string }) {
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4" role="dialog" aria-modal="true">
          ...
          <div className="relative w-full max-w-sm rounded-xl border border-zinc-200 bg-white shadow-xl dark:border-zinc-700 dark:bg-zinc-900">
            <div className="flex flex-col items-center px-6 pt-8 pb-6 text-center">
              ...
              <h2 className="mb-1 text-base font-semibold text-zinc-900 dark:text-zinc-50">Remove domain?</h2>
              <p className="text-sm text-zinc-500 dark:text-zinc-400">
                <span className="font-mono font-medium text-zinc-700 dark:text-zinc-300">{label}</span> will no longer allow signups.
              </p>
            </div>
            <div className="flex gap-3 border-t border-zinc-100 px-6 py-4 dark:border-zinc-800">
              ...
              <form action={deleteDomain} className="flex-1">
                <input type="hidden" name="id" value={id} />
                <button type="submit" className="w-full cursor-pointer rounded-lg bg-red-600 px-4 py-2 text-sm font-medium text-white transition-colors hover:bg-red-700">Remove</button>
              </form>
            </div>
          </div>
        </div>
