...
interface Props {
  action: (prevState: ProviderFormState, formData: FormData) => Promise<ProviderFormState>;
  defaultValues?: { id?: number; name?: string };
}

export function ProviderForm({ action, defaultValues = {} }: Props) {
  ...
  return (
    <form action={formAction} className="max-w-sm space-y-6">
      ...
      <div className="flex items-center gap-3">
        ...
        <Link href="/admin/llm-providers" className="text-sm font-medium text-zinc-500 hover:text-zinc-700 dark:text-zinc-400 dark:hover:text-zinc-200">
          Cancel
        </Link>
      </div>
    </form>
