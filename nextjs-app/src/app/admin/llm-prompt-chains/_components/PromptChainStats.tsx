...
async function fetchStats() {
  ...
  const [totalRes, last30Res, last7Res] = await Promise.all([
    ...
    supabase.from("llm_prompt_chains").select("*", { count: "exact", head: true }).gte("created_datetime_utc", thirtyDaysAgo),
    supabase.from("llm_prompt_chains").select("*", { count: "exact", head: true }).gte("created_datetime_utc", sevenDaysAgo),
  ]);
  return {
...
export async function PromptChainStats() {
  const { total, last30, last7 } = await fetchStats();
  return (
    <div className="mb-8 grid grid-cols-2 gap-4 sm:grid-cols-3">
      <StatCard label="Total Chains" value={total} />
