...
import type { EmailFormState } from "../actions";

interface Props {
  action: (prevState: EmailFormState, formData: FormData) => Promise<EmailFormState>;
  defaultValues?: { id?: number; email_address?: string };
}

export function EmailForm({ action, defaultValues = {} }: Props) {
  ...
  return (
    <form action={formAction} className="max-w-sm space-y-6">
      ...
      <div className="flex items-center gap-3">
        ...
        <Link href="/admin/whitelisted-emails" className="text-sm font-medium text-zinc-500 hover:text-zinc-700 dark:text-zinc-400 dark:hover:text-zinc-200">Cancel</Link>
      </div>
    </form>
  );
}
